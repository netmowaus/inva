...
<head>
    ...
    <title>Add Payment for Invoice #{{ $invoice->invoice_number }}</title>
    <style>
    ...
    <div class="container">
        ...
        <form action="{{ route('admin.invoices.payments.store', $invoice) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="number" id="amount" name="amount" value="{{ old('amount', number_format($remainingBalance, 2, '.', '')) }}" step="0.01" min="0.01" max="{{number_format($remainingBalance, 2, '.', '')}}" required>
            </div>

            <div class="form-group">
                <label for="payment_method">Payment Method (Optional):</label>
                <input type="text" id="payment_method" name="payment_method" value="{{ old('payment_method') }}" placeholder="e.g., Bank Transfer, Credit Card, Cash">
            </div>
